...
def get_target_classes_str(dept, grade, dept_class_map):
    """
    根據系所、年級，以及現有班級資料，組合出目標班級字串。
    例如回傳: "藥學一甲.藥學一乙.藥學一丙" 或 "多媒體三甲"
    """
    grade_map = {"1": "一", "2": "二", "3": "三", "4": "四", "5": "五"}
    g_char = grade_map.get(grade, grade)
    
    # 取得該系所與年級的所有班級
    classes = dept_class_map.get(dept, {}).get(grade, set())
    
    if classes:
        # 將班級排序 (甲, 乙, 丙...)
        sorted_classes = sorted(list(classes))
        # 組合字串並以 "." 連接
        class_list = [f"{dept}{g_char}{c}" for c in sorted_classes]
        return ".".join(class_list)
    else:
        # 若原資料中沒有分班，則預設輸出 (例如: "高福一")
        return f"{dept}{g_char}"
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for line in text.split('\n'):
                ...
                match = re.search(r'(一|二|三|四)年級[:：]\s*(.*)', line)
                if match:
                    g_char = match.group(1)
                    g_num = {"一": "1", "二": "2", "三": "3", "四": "4"}.get(g_char, "1")
...
def main():
    ...
    # 3. 自動尋找目錄下所有的 PDF 開課表 (支援 W1 到 W5)
            ...
            for page in pdf.pages:
                ...
                for line in lines:
                    ...
                    if match:
                        ...
                        # 4. 針對表頭抓取到的每一個系所，分別建立獨立的課程資料
                        for target in target_depts:
                            dept_name = target['dept']
                            grade_num = target['grade']
                            
                            # 自動生成目標班級字串 (如: 藥學一甲.藥學一乙.藥學一丙)
                            target_class_str = get_target_classes_str(dept_name, grade_num, dept_class_map)
                            ...
                            new_course = {
                                ...
                                "required_or_elective": "發展通識", # 依照你的需求設定為發展通識
    ...
    print(f"\n執行完成！共新增了 {len(new_courses)} 筆課程記錄，已儲存至 {output_json_path}")
